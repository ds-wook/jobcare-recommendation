...
from model.boosting import LightGBMTrainer, XGBoostTrainer
...
class LightGBMTuner(BaseTuner):
    ...
    def _objective(self, trial: FrozenTrial, config: DictConfig) -> float:
        ...
        lgbm_trainer = LightGBMTrainer(
            run=pruning_callback, search=True, config=config, metric=f1_score
        )
        result = lgbm_trainer.train(self.train_x, self.train_y)
        score = f1_score(
            self.train_y.to_numpy(), result.oof_preds > self.config.model.threshold
        )
